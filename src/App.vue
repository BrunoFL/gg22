<template>
  <div class="container-fluid" id="app">
    <Lobby
      v-if="isInLobby"
      @sendLobbyToApp="(event) => getLobbyData(event)"
      @sendPlayerToApp="(event) => getPlayerData(event)"
    ...
    <Games
      ...
      @swapToGames="(event) => swapToGames(event)"
      :listGames="listGames"
    ></Games>
    <Anvil
      v-if="isInAnvilGame"
      :lobby="lobby"
      @swapScreen="(event) => swapScreenFromAnvil(event)"
    ></Anvil>
    <GameRanking v-if="false"></GameRanking>
    <HundredMetersClick
      v-if="isInHundredMetersClic"
      @swapScreen="(event) => swapScreenFromHundred(event)"
    ></HundredMetersClick>
    ...
    <Obstacle
      v-if="isInObstacleRun"
      @swapScreen="(event) => swapScreenFromObstacle(event)"
    ></Obstacle>
  </div>
</template>
...
export default {
  ...
  methods: {
    swapScreen(listGames) {
      (this.isInLobby = false), (this.isInGamesList = true);
      this.listGames = listGames;
    },
    swapScreenFromAnvil(listGames) {
      (this.isInAnvilGame = false), (this.isInGamesList = true);
      this.listGames = listGames;
    },
    swapScreenFromHundred(listGames) {
      (this.isInHundredMetersClic = false), (this.isInGamesList = true);
      this.listGames = listGames;
    },
